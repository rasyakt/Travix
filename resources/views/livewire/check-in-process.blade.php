<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    @if(!$canCheckIn)
        <div
            ...
            <span><strong>Check-in not available.</strong> {{ $checkInBlockedReason ?? 'Please check your booking status and flight departure time.' }}</span>
        </div>
    @endif
    ...
    <div class="tv-card p-5 mb-5">
        ...
        <div class="mt-6">
            <button wire:click="processCheckIn" @if(!$canCheckIn || empty($selectedPassengers)) disabled @endif
                ...
                @else
                    Check In {{ count($selectedPassengers) }} {{ \Illuminate\Support\Str::plural('Passenger', count($selectedPassengers)) }}
                @endif
            </button>
        </div>
    </div>
